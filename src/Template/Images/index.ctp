...
<?php echo $this->Html->css('dropzone');
 ...
 $urlRedirectToIndex = $this->Url->build(["controller" => "Images","action" => "index"]);
...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?php //= $this->Html->link(__('New Image'), ['action' => 'add']) ?></li>
    </ul>
</nav>
-->
...
<div class="image_upload_div large-9 medium-8 columns content">

    <?php  echo $this->Form->create('image',array('url'=>array('controller'=>'Images','action'=>'drop'),
    'method'=>'post',
